...
import ProjectCardSkeleton from "../../manager/_components/project-card-skeleton";
import UpdateProgress from "../../manager/_components/update-progress";
...
const MyProjects = () => {
  ...
  const myId = (session?.user as SessionUser)?.id;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {showSkeleton
          ...
          : projects.map((project) => {
              // Find my progress data from approved engineers
              const myEngineerData = project.approvedEngineers.find(
                ({ engineer }) => engineer._id === myId
              );
                              ...
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                  <span className="text-sm font-medium truncate">
                                    {myEngineerData.engineer.firstName}{" "}
                                    {myEngineerData.engineer.lastName}
                                  </span>
                                  {myEngineerData.isManager && (
                                ...
                                <span className="text-sm text-[#147575]">
                                  {(myEngineerData as any)?.progress}%
                                </span>
                              </div>
                              <Progress
                                value={(myEngineerData as any)?.progress}
                                className="h-2"
                              />
                            </div>
